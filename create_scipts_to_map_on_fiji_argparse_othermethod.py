...
def createbowtieindexs(bowtieindir="/scratch/Shares/public/genomes/"):
	indirfiles=[]
	bowtieindexdic = {}
	for root, dirnames, filenames in os.walk(bowtieindir):
		for filename in fnmatch.filter(filenames, '*genome.1.bt2'):
        		indirfiles.append(os.path.join(root, filename))
			genomename=root.split("/")[-3]
			bowtiename = filename[:-6]
			bowtieindexdic[genomename] = os.path.join(root, bowtiename) 	
	return bowtieindexdic

bowtieindexdic = createbowtieindexs(bowtieindir="/scratch/Shares/public/genomes/") 
...
def bowtie2file(passfile, outdir, wfname):
	...
	bowtieindex = bowtieindexdic[args.genome]
...
def sortedbamtobedgraphfile(sortedbam, outdir, wfname):
	...
	wf.write("genomeCoverageBed -bg -strand + -ibam "+sortedbam+" -g "+args.genome+" > "+Bedgraphoutdir+rootname+".pos.BedGraph\n")
	wf.write("genomeCoverageBed -bg -strand - -ibam "+sortedbam+" -g "+args.genome+" | awk -F '\t' -v OFS='\t' '{ $4 = - $4 ; print $0 }' > "+Bedgraphoutdir+rootname+".neg.BedGraph\n")
...
def igvcreatefile(readcountcorrBedgraphfile, outdir, wfname):
	...
	igvgenomefile = igvdir+"/genomes/"+args.genome+".chrom.sizes" 
